...
class TelemetryControl():
	def __init__(self, registry: Registry, flag: Flag):
		...
		self.arguments = {
			...
			"soft_abort": (),
			"solenoid_actuate": (
			("valve_location", ValveLocation), ("actuation_type", ActuationType), ("priority", int)),
	...
	def solenoid_actuate(self, valve_location: ValveLocation, actuation_type: ActuationType, priority: int) -> Error:
		err, current_priority, timestamp = self.registry.get(
			("valve_actuation", "actuation_priority", ValveType.SOLENOID, valve_location), allow_error=True)
		if err != Error.NONE:
	...
	def sensor_request(self, sensor_type: SensorType, sensor_location: SensorLocation) -> Error:
		...
		_, kalman_value, timestamp = self.registry.get(("sensor_normalized", sensor_type, sensor_location),
		                                               allow_error=True)
		if err != Error.NONE:
			return Error.REQUEST_ERROR
		_, status, _ = self.registry.get(("sensor_status", sensor_type, sensor_location))
		log = Log(header="sensor_data",
		          message={"type": sensor_type, "location": sensor_location, "value": (value, kalman_value),
	...
	def valve_request(self, valve_type: ValveType, valve_location: ValveLocation) -> Error:
		...
		_, actuation_type, timestamp = self.registry.get(
			("valve_actuation", "actuation_type", valve_type, valve_location))
		_, actuation_priority, _ = self.registry.get(("valve", "actuation_priority", valve_type, valve_location))
		log = Log(header="valve_data", message={"type": valve_type, "location": valve_location, "state": value,
		                                        "actuation_type": actuation_type,
		                                        "actuation_priority": actuation_priority,
		                                        "actuation_timestamp": timestamp})
		self.enqueue(log, LogPriority.INFO)
